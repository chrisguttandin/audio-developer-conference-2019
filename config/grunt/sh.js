module.exports = (grunt) => {
    ...
    return {
        ...
        'build': {
            cmd: 'npx ng build --base-href /audio-developer-conference-2019/ --configuration production --subresource-integrity'
        },
        'continuous': {
            cmd: 'npx ng test'
        },
        ...
        'lint-src': {
            cmd: `htmlhint --config config/htmlhint/document.json 'src/**/index.html' && \
                ...
                npx ng lint audio-developer-conference-2019 && \
                npx stylelint 'src/**/*.scss' --config config/stylelint/config.json`
        },
        'lint-test': {
            cmd: 'npx ng lint audio-developer-conference-2019 --configuration test'
        },
        'monitor': {
            cmd: 'npx ng serve'
        },
        ...
        'rimraf-source-maps': {
            cmd: 'rimraf build/audio-developer-conference-2019/browser/**.map'
        },
        'smoke': {
            cmd: `npx playwright install --with-deps && \
        ...
        'verify': {
            cmd: `npx bundle-buddy build/audio-developer-conference-2019/browser/*.js.map && \
                grep -r build/audio-developer-conference-2019/browser/*.js.map -e '/environments/environment.ts'; test $? -eq 1`
        }
    };
};
