...
const replaceHashInMatch = (grunt, match, prefix, index) => {
    const filename = grunt.file.expand({ cwd: 'build/audio-developer-conference-2019/scripts' }, `${ prefix }.*.js`)[0];

    if (filename === undefined) {
        return match;
    }

    const hash = `sha384-${ computeHashOfFile(`build/audio-developer-conference-2019/scripts/${ filename }`, 'sha384', 'base64') }`;
...
module.exports = (grunt) => {
    return {
        ...
        'chunks': {
            files: {
                './': [
                    'build/audio-developer-conference-2019/index.html'
                ]
            },
            options: {
                patterns: [ {
        ...
        'csp-production': {
            ...
            options: {
                patterns: [ {
                    ...
                    replacement: () => {
                        const html = fs.readFileSync('build/audio-developer-conference-2019/index.html', 'utf-8'); // eslint-disable-line node/no-sync
        ...
        'manifest': {
            files: {
                './': [
                    'build/audio-developer-conference-2019/ngsw.json'
                ]
            },
            options: {
                patterns: [ {
                    match: /assets\/(?<filename>[\da-z-]+)\.(?<extension>ico|jpg|png)/g,
                    replacement: (_, filename, extension) => grunt.file.expand({ cwd: 'build/audio-developer-conference-2019', ext: extension }, `assets/${ filename }.*`)[0]
                }, {
                    ...
                    match: /\s*"\/audio-developer-conference-2019(?:\/scripts)?\/runtime(?:-es(?:2015|5))?.[\da-z]*\.js",/g,
                    replacement: ''
                }, {
                    match: /\s*"\/audio-developer-conference-2019(?:\/scripts)?\/runtime(?:-es(?:2015|5))?.[\da-z]*\.js":\s"[\da-z]+",/g,
                    replacement: ''
                }, {
                    // Replace the hash value inside of the hashTable for "/scripts/*.js" because it may have been modified before.
                    match: /"\/audio-developer-conference-2019(?<filename>\/scripts\/(?:\d+|main|scripts)(?:-es(?:2015|5))?.[\da-z]+.js)":\s"[\da-z]+"/g,
                    replacement: (_, filename) => {
                        return `"/audio-developer-conference-2019${ filename }": "${ computeHashOfFile(`build/audio-developer-conference-2019${ filename }`, 'sha1', 'hex') }"`;
                    }
                }, {
                    // Replace the hash value inside of the hashTable for "/styles/styles*.css" because it was modified before.
                    match: /"\/audio-developer-conference-2019(?<filename>\/styles\/styles\.[\da-z]*\.css)":\s"[\da-z]+"/g,
                    replacement: (_, filename) => {
                        return `"/audio-developer-conference-2019${ filename }": "${ computeHashOfFile(`build/audio-developer-conference-2019${ filename }`, 'sha1', 'hex') }"`;
                    }
                }, {
                    // Replace the hash value inside of the hashTable for "/(index|start).html" because it was modified before.
                    match: /"\/audio-developer-conference-2019\/(?<filename>index|start)\.html":\s"[\da-z]+"/g,
                    replacement: (_, filename) => {
                        return `"/audio-developer-conference-2019/${ filename }.html": "${ computeHashOfFile(`build/audio-developer-conference-2019/${ filename }.html`, 'sha1', 'hex') }"`;
                    }
                } ]
            }
        },
                ...
                patterns: [ {
                    ...
                    replacement: (match, filename, moduleAttribute) => {
                        if (moduleAttribute === undefined) {
                            return `<script>${ fs.readFileSync(`build/audio-developer-conference-2019/${ filename }`) }</script>`; // eslint-disable-line node/no-sync
                        }

                        return `<script${ moduleAttribute }>${ fs.readFileSync(`build/audio-developer-conference-2019/${ filename }`) }</script>`; // eslint-disable-line node/no-sync
                    }
                } ]
            }
        },
                ...
                patterns: [ {
                    ...
                    replacement: (match, filename, moduleAttribute, initialHash) => {
                        const updatedHash = (/main(?:-es(?:2015|5))?\.[\da-z]+\.js/.test(filename)) ?
                            `sha384-${ computeHashOfFile(`build/audio-developer-conference-2019/scripts/${ filename }`, 'sha384', 'base64') }` :
                            initialHash;
            ...
            options: {
                patterns: [ {
                    ...
                    replacement: (match, filename) => {
                        const hash = `sha384-${ computeHashOfFile(`build/audio-developer-conference-2019/styles/${ filename }`, 'sha384', 'base64') }`;
